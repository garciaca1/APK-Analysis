...
from flask import Flask, render_template, request, send_file, flash, redirect, url_for, g
...
def fetch_android_permissions():
    ...
    try:
        ...
        logging.info(f"Received response: {response.status_code}, length: {len(response.text)}")
        ...
        dangerous_permissions = {
            ...
            "ANSWER_PHONE_CALLS", "ACCESS_BACKGROUND_LOCATION", "READ_MEDIA_AUDIO",
        ...
        for row in table.find_all("tr")[1:]:  
            ...
            if len(cols) < 2:
                logging.warning(f"Skipping row with insufficient columns: {row.text[:100]}")
                continue
            ...
            logging.info(f"Extracted permission: {full_perm_name}, Category: {level}, Description: {description[:50]}...")
...
def load_permissions():
    ...
    try:
        ...
        logging.info(f"Loaded {len(permissions)} permissions from Android Developers site")
        return permissions
    except Exception as e:
...
def load_permissions_before_request():
    ...
    if not hasattr(g, 'permissions'):
        ...
        g.dangerous_permissions = {perm for perm, data in g.permissions.items() if data['category'] == "Dangerous"}
        g.permission_descriptions = {perm: data['description'] for perm, data in g.permissions.items()}
        logging.info(f"Loaded {len(g.permissions)} permissions, {len(g.dangerous_permissions)} dangerous")
        logging.info(f"Permission descriptions (first few): {list(g.permission_descriptions.items())[:3]}")
...
def decompile_apk(apk_path, output_folder):
    ...
    try:
        ...
        with open(os.path.join(output_folder, "jadx_output.log"), "w", encoding="utf-8") as log_file:
...
def get_api_level(manifest_path):
    ...
    try:
        ...
        if uses_sdk is not None:
            api_level = uses_sdk.attrib.get("{http://schemas.android.com/apk/res/android}targetSdkVersion")
...
def get_apk_signature(apk_path):
    ...
    try:
        with zipfile.ZipFile(apk_path, "r") as apk_zip:
            signature_files = [f for f in apk_zip.namelist() if f.startswith("META-INF/") and f.endswith(".RSA")]
            return ", ".join(signature_files) if signature_files else "No signature found"
    except Exception as e:
    ...
    try:
        ...
        for perm in root.findall("uses-permission"):
            ...
            if perm_name:
                ...
                category = "Dangerous" if perm_name in g.dangerous_permissions else "Normal"
                logging.info(f"Permission extracted: {original_perm_name} -> Normalized: {perm_name}, Category: {category}")
        ...
        if app_section:
            for activity in app_section.findall("activity"):
                name = activity.attrib.get("{http://schemas.android.com/apk/res/android}name", "Unknown")
                exported = activity.attrib.get("{http://schemas.android.com/apk/res/android}exported")
                exported_status = "TRUE" if exported and exported.lower() == "true" else "FALSE"
                activities.append((name, exported_status))
            for provider in app_section.findall("provider"):
                name = provider.attrib.get("{http://schemas.android.com/apk/res/android}name", "Unknown")
                exported = provider.attrib.get("{http://schemas.android.com/apk/res/android}exported")
                exported_status = "TRUE" if exported and exported.lower() == "true" else "FALSE"
                providers.append((name, exported_status))
            for service in app_section.findall("service"):
                name = service.attrib.get("{http://schemas.android.com/apk/res/android}name", "Unknown")
                exported = service.attrib.get("{http://schemas.android.com/apk/res/android}exported")
                exported_status = "TRUE" if exported and exported.lower() == "true" else "FALSE"
...
def compute_summary_stats(permissions, activities, providers, services, api_level, signature_info):
    """Computes summary statistics for the component usage heatmap."""
    summary = {
        'API Level': {'Total Found': api_level, 'Exported': '–', 'Dangerous': '–'},
        'APK Signature': {'Total Found': signature_info, 'Exported': '–', 'Dangerous': '–'},
        'Permissions': {'Total Found': len(permissions), 'Exported': '–', 'Dangerous': sum(1 for _, cat in permissions if cat == 'Dangerous')},
        'Activities': {'Total Found': len(activities), 'Exported': sum(1 for _, exp in activities if exp == 'TRUE'), 'Dangerous': '–'},
        'Services': {'Total Found': len(services), 'Exported': sum(1 for _, exp in services if exp == 'TRUE'), 'Dangerous': '–'},
        'Providers': {'Total Found': len(providers), 'Exported': sum(1 for _, exp in providers if exp == 'TRUE'), 'Dangerous': '–'},
    }
    return summary
    ...
    try:
        with open(csv_filename, mode="w", newline="", encoding="utf-8") as csvfile:
            ...
            for prov_string in providers_from_strings:
                writer.writerow(["Provider (String Reference)", prov_string, "N/A", "N/A"])
            for serv_string in services_from_strings:
                writer.writerow(["Service (String Reference)", serv_string, "N/A", "N/A"])
...
def filter_csv(analysis_folder):
    ...
    try:
        ...
        exported_df = df[(df["Exported"] == "TRUE") & (df["Type"].isin(["Activity", "Provider", "Service"]))]
        if exported_df.empty:
            logging.info("No exported components found! Check the Exported column formatting.")
        exported_df.to_csv(os.path.join(analysis_folder, "exported_components.csv"), index=False)
        ...
        if not permissions_df.empty:
            ...
            permissions_df['Description'] = permissions_df['Value'].map(g.permission_descriptions).fillna('No description available')
            logging.info(f"Permissions DataFrame after adding Description:\n{permissions_df.to_string()}")
        else:
            logging.info("No permissions found.")
            permissions_df = pd.DataFrame(columns=["Type", "Value", "Exported", "Category", "Description"])
        permissions_df.to_csv(os.path.join(analysis_folder, "permissions.csv"), index=False)
        logging.info(f"Permissions CSV written to: {os.path.join(analysis_folder, 'permissions.csv')}")
        ...
        logging.info("Filtered CSVs have been created successfully in %s", analysis_folder)
    except Exception as e:
...
def analyze_apk(filename, apk_path, analysis_folder):
    ...
    if os.path.exists(analysis_folder):
        ...
        if os.path.exists(summary_stats_file):
            ...
            flash(f"Analysis for {filename} already exists. Retrieving existing results.", "info")
            return {
                'apk_name': filename,
            ...
            flash(f"Analysis folder for {filename} exists but is incomplete. Re-running analysis.", "warning")
    ...
    if not manifest_path:
        ...
        with open(os.path.join(analysis_folder, "metadata.json"), 'w') as f:
            json.dump(metadata, f)
        flash(f"AndroidManifest.xml not found for {filename}. Analysis cannot continue.", "danger")
        return None
    ...
    strings_found, providers_from_strings, services_from_strings = find_hardcoded_strings(analysis_folder)
    export_to_csv(
    ...
    summary_stats = compute_summary_stats(permissions, activities, providers, services, api_level, signature_info)
...
def index():
    ...
    if hasattr(g, 'permissions_status'):
        if g.permissions_status == "using_outdated":
            flash("Failed to fetch the latest Android permissions list. Using outdated local permissions.", "warning")
        elif g.permissions_status == "using_default":
            flash("Failed to fetch Android permissions list and no local permissions found. Using default permissions list.", "warning")
        elif g.permissions_status == "using_default_error":
            flash("Error loading Android permissions. Using default permissions list.", "danger")
    ...
    if request.method == 'POST':
        ...
        for file in valid_files:
            ...
            analysis_folder = os.path.join(app.config['ANALYSIS_FOLDER'], analysis_folder_name)
...
def download_zip(analysis_folder):
    ...
    zip_path = os.path.join(app.config['ANALYSIS_FOLDER'], f"{analysis_folder}_analysis.zip")
    with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
        for root, _, files in os.walk(analysis_path):
            for file in files:
                if file != "metadata.json" and file != "summary_stats.json":  
                    file_path = os.path.join(root, file)
                    arcname = os.path.relpath(file_path, analysis_path)
                    zipf.write(file_path, arcname)

    return send_file(zip_path, as_attachment=True, download_name=f"{analysis_folder}_analysis.zip")
...
def view_csv(analysis_folder, filename):
    ...
    csv_data = read_csv_to_json(os.path.join(app.config['ANALYSIS_FOLDER'], analysis_folder), filename)
    ...
    title = next((title for file, title, _ in [
        ...
        ('permissions.csv', 'All Permissions', 'All permissions with descriptions (dangerous and normal)'),
    ...
    return render_template('view_csv.html', csv_data=csv_data, title=title, analysis_folder=analysis_folder)
